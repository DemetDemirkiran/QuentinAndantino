...
class Map(object):
    ...
    def cells(self):
        cells = []
        for row in np.arange(int(self.rows + self.cols / 2)):
            cells.extend(
...
if __name__ == '__main__':
    ...
    parser.add_argument('-r', '--rows', dest='rows', type=int, default=5,
                        help='Number of rows in grid.  Defaults to 5.')
    parser.add_argument('-c', '--cols', dest='cols', type=int, default=5,
                        help='Number of columns in grid.  Defaults to 5.')
    parser.add_argument('-n', '--numbers', action="store_true", dest="numbers", default=True,
